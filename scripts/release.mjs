// scripts/release.mjs
//
// ⚠️ Diese Kopie ist NICHT mehr byte-identisch mit den Schwester-Kopien im Dach: dort steht
// in Kommentaren und Log-Meldungen noch „Codeberg", wo seit dem Umzug auf git.jkaindl.de
// Forgejo gemeint ist (der Code war längst umgestellt — Regex, ~/.forgejo-token,
// createForgeRelease —, nur die Wörter nicht). Hier bewusst korrigiert, weil die Meldung
// den Maintainer sonst beim Release anlügt. Acht weitere Kopien tragen den Alt-Text noch;
// die Angleichung gehört ins Dach (mit der Zentralisierung nach tools/release/release.mjs).
//
